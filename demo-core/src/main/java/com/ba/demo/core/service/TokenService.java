...
public class TokenService {
  ...
  @NonNull private static final Logger LOGGER = LoggerFactory.getLogger(TokenService.class);
  @NonNull private final JwtService jwtService;
  @NonNull private final TimeService timeService;
  @NonNull private final Supplier<UUID> identitySupplier = () -> UUID.randomUUID();
  @NonNull private final TokenRepository tokenRepository;
  @NonNull private final TokenConfig config;
  @NonNull private final SaltService saltService;
  @NonNull private final UserRepository abstractUserRepository;
  @NonNull private final InternationalizatonService internationalizatonService;

  @NonNull private final CustomUserRepository genericUserService;
  ...
  public MutablePair<UserDTO, Collection<? extends GrantedAuthority>> verifyLoginToken(
      final String tokenEncrypted) throws UserNotFoundException, ExecutionException {
  ...
  @Transactional
  public TokenResponseDTO createUserToken(final TokenRequestDTO request)
    ...
    return byUserName
        .filter(
            user ->
                this.saltService.check(user.getSalt(), user.getPassword(), request.getPassword()))
        .map(
            user -> {
  ...
  //        Optional<TokenForgot> token =
  // this.tokenForgotRepository.findById(Identity.unsafeFromString(tokenString));
  ...
  //    @Transactional
  //    public void createUserForgotToken(ForgotRequestDTO request, Boolean isWebUser) throws
  // UserNotFoundException {
  //        LOGGER.debug("Creating user forgot token: {}", request);
  //        Optional<AbstractUser> byUserName =
  // this.abstractUserRepository.findByEmail(request.getEmail());
  ...
  //                    if (isWebUser)
  //                        tokenForgot = new TokenForgot(identitySupplier.get(), user.getId(),
  // timeService.now(), timeService.after(config.activationTokenTtlDays()), isWebUser, null);
  //                    else
  //                        tokenForgot = new TokenForgot(identitySupplier.get(), user.getId(),
  // timeService.now(), timeService.after(config.activationNumberTtlSeconds()), isWebUser,
  // Utils.generateRandomNumber(this.config.getActivationNumberLength()));
  ...
  //
  //        String subject =
  // internationalizatonService.getByLanguageId(byUserName.get().getLanguage(),
  // "recovery_password_subject");
  //
  //        if (isWebUser) {
  //            String content =
  // internationalizatonService.getByLanguageId(byUserName.get().getLanguage(),
  // "code_verification_body_web");
  //            String buttonText =
  // internationalizatonService.getByLanguageId(byUserName.get().getLanguage(), "confirm");
  ...
  //            this.emailService.sendNotification(request.getEmail(), subject,
  // "templates/email/confirmation.code.html", params);
  //        } else {
  //            String content =
  // internationalizatonService.getByLanguageId(byUserName.get().getLanguage(),
  // "code_verification_body");
  //            Map<String, Object> params = ImmutableMap.of("title", subject,
  //                    "body", content, "confirmationCode", result.getToken());
  //            emailService.sendNotification(request.getEmail(), subject,
  // "templates/email/confirmation.code.mobile.html", params);
  //        }
  //    }

  //    @Transactional
  //    public void changePassword(ForgotChangePasswordRequestDTO requestDTO) throws
  // UserNotFoundException {
  ...
  private TokenResponseDTO findOrCreateToken(UserEntity user) {
    TokenEntity tkn =
        new TokenEntity(
            ...
            user.getId(),
            timeService.now(),
            timeService.after(config.tokenTtlDays()));
