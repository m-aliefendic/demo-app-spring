...
public class ControllerErrorHandler {
  ...
  public static final String VALIDATION_PROBLEM_TITLE =
      "There are some validation error(s) while processing the request";
  ...
  public ResponseEntity<Problem> handleAnyException(EntityNotFoundException ex) {
    ...
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(
            new Problem(
                ErrorCode.NOT_FOUND.getMessage(), MessagesKey.entity_not_found.getName(), null));
  }
  ...
  public ResponseEntity<Problem> handleCustomException(SpecificException ex) {
    ex =
        new SpecificException(
            ex,
            internationalizatonService.get(ex.getMessageKey().getName(), ex.getMessageTokens()));
  ...
  public ResponseEntity<Problem> handleTypeMismatchException(
  ...
  private ResponseEntity<Problem> respond(
      ErrorCode errorCode, MessagesKey messageKey, Object... messageTokens) {
    Problem error =
        new Problem(
  ...
  public static Problem convert(final MethodArgumentNotValidException ex, final String message) {
    ...
    final List<InvalidParam> fieldValidationErrors =
        fieldErrors.stream()
            .map(
                fieldError ->
                    new InvalidParam(
                        ...
                        ValidationMessages.reason(fieldError.getDefaultMessage())))
