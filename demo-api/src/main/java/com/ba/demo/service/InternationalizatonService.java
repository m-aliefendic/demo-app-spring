...
public class InternationalizatonService {
  ...
  // Get language by token; when token/language is not found, get by LocaleContextHolder - lang
  // param
      ...
      if (userDetails != null) {
        List<Language> lang =
            languageService.getAll().stream()
                .filter(x -> x.getCode().equals(userDetails.getLanguage()))
